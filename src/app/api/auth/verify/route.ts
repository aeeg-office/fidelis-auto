...
export async function POST(request: Request) {
  try {
    ...
    if (user.verificationCode !== code) return NextResponse.json({ error: "Wrong code" }, { status: 400 });
    if (new Date() > user.verificationCodeExpiry) return NextResponse.json({ error: "Code expired. Please resend." }, { status: 400 });
